...
export class AuthService extends HttpBaseService {
  ...
  login(request: LoginRequest): Observable<ApiResponse<LoginResponse>> {
    ...
    return this.http.post<ApiResponse<LoginResponse>>(url, request).pipe(
      ...
      tap(() => this.introspect(this.credentials.token).subscribe()),
  ...
  saveUserProfile(userProfile: UserCredentials): void {
    const res = userProfile.userProfile;
  ...
  getPointsCurrent(): Observable<PointsCurrent> {
    return this.http
      .get<ApiResponse<PointsCurrent>>(
        `${this.baseUrl}/user/get-points-current`
      )
