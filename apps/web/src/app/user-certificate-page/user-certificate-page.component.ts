...
@Component({
  selector: 'app-user-certificate-page',
  templateUrl: './user-certificate-page.component.html',
  styleUrls: ['./user-certificate-page.component.less'],
})
export class UserCertificatePageComponent implements OnInit {
  ...
  fetchCertificates() {
    this.certificateService.getAllCertificatesForCurrentUser().subscribe({
      next: (res: CertificateResponse[]) => {
  ...
  handleDownload(certificateId: number) {
    Swal.fire({
